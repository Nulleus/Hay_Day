...
public class Preload : MonoBehaviour
{
    ...
    {
        RestClient.Post<ResponseFileDataBase>("http://89.110.90.192/api/dump_sql.php", new POSTFileDataBase
        {
    ...
    void GetSQLQueryFull()
    {
        ...
        StartCoroutine(postRequest("http://89.110.90.192/api/dump_sql.php", body));
    }
    [Button(ButtonSizes.Medium, ButtonStyle.FoldoutButton)]
   ...
   //Получаем серверное время
    ...
    {
        RestClient.Post<ResponseGetDateTimeNowServer>("http://89.110.90.192/api/preload_execute_methods.php", new POSTGetDateTimeNowServer
        {
            jwt = Data.GetComponent<User>().GetJWTToken(),
    ...
    void Update()
    {
        if (Check)
        {
            ...
            {
                SetSQLQueryFull();
                //Тут должно быть действие о завершении загрузки
                Check = false;
            }
